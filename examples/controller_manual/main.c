...
static device_info_t g_devices[DEVICE_CONFIG_COUNT];
...
static int probe_device(device_info_t *d)
{
    int crumbs_ok;
    int module_ok;
    uint8_t exp_major = 0;
...
static int cmd_probe(void)
{
    ...
    for (i = 0; i < g_device_count; i++)
    {
        ...
        printf("[0x%02X] %s\n", d->dev.addr, type_name(d->type_id));
...
static void print_help(void)
{
    ...
    printf("  probe                         - probe configured devices\n");
    printf("  scan                          - alias for probe\n");
...
static void cmd_list(void)
{
    ...
    for (i = 0; i < g_device_count; i++)
    {
        ...
        printf("  [%d] %s #%d @0x%02X compat=%s\n",
               ...
               (g_devices[i].compat == 0) ? "OK" : "UNKNOWN/NO");
    }
    printf("\n");
}
    ...
    if (*args == '@')
    {
        ...
        for (i = 0; i < g_device_count; i++)
        {
            if (g_devices[i].dev.addr == (uint8_t)addr && g_devices[i].type_id == type_id)
            {
                ...
                {
                    printf("Device at 0x%02X is not compatible/probed. Run 'probe'.\n", (uint8_t)addr);
                    return -1;
                }
                *out_dev = &g_devices[i];
                *out_rest = rest;
...
static void init_configured_devices(crumbs_context_t *ctx, crumbs_linux_i2c_t *lw)
{
    size_t i;

    g_device_count = (int)DEVICE_CONFIG_COUNT;

    for (i = 0; i < DEVICE_CONFIG_COUNT; i++)
    {
        device_info_t *d = &g_devices[i];
        memset(d, 0, sizeof(*d));
        d->type_id = DEVICE_CONFIG[i].type_id;
        d->dev.addr = DEVICE_CONFIG[i].addr;
        d->dev.ctx = ctx;
...
int main(int argc, char **argv)
{
    ...
    init_configured_devices(&ctx, &lw);

    printf("BREAD manual controller on %s\n", i2c_path);
    print_help();
    cmd_probe();
